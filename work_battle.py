...
class AI(Player):
    def ask(self):
        ...
        print(f"Ход компьютера: {chr(ord('A') + d.y)}{d.x}")
        return d
...
class User(Player):
    def ask(self):
        while True:
            cords = input("Ваш ход (например, A1): ").strip().upper()
            
            if len(cords) < 2:
                print("Введите координаты в формате 'буква цифра' (например, A1)")
                continue
            
            # Извлекаем букву и цифры
            letter = cords[0]
            number_str = cords[1:]
            
            # Проверяем, что буква от A до J
            if letter < 'A' or letter > 'J':
                print("Буква должна быть от A до J")
                continue
            
            # Проверяем, что цифра корректна
            if not number_str.isdigit():
                print("После буквы должны быть цифры")
                continue
...
class Game:
    ...
    def greet(self):
        ...
        print("    морской бой    ")
        print("-------------------")
        print(" формат ввода: буква цифра")
        print(" например: A1, B5, C0")
        print(" буква - столбец (A-J)")
        print(" цифра - строка (0-9)")
